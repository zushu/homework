...
void test()
{
  ...
  new_dentry->d_name = "temp";
  new_inode->i_op->lookup(new_inode, new_dentry);

  printf("temp finished\n");
  new_inode->i_ino = new_dentry->d_inode->i_ino;
  current_sb->s_op->read_inode(new_inode);
  struct dentry* another_dentry = malloc(sizeof(struct dentry));
  another_dentry->d_name = malloc(8*sizeof(char));
  another_dentry->d_name = "dummy";
  printf("new lookup\n");
  new_inode->i_op->lookup(new_inode, another_dentry);
  printf("new lookup finished\n");
  printf("parent name: %s\n", another_dentry->d_parent->d_name);
  free(new_inode);
  free(new_dentry);
  free(another_dentry);
}
